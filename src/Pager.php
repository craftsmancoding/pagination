...
class Pager {
	...
	private function _parse_firstTpl() {
		if ($this->offset > 0) {
			return $this->_parse(static::$tpls['firstTpl'], array('offset'=> 0, 'page_number'=> 1 ));
		} else {
			return '';
		}
	}
	...
	private function _parse_lastTpl() {
		...
		if ($this->current_page < $this->page_count) {
			return $this->_parse(static::$tpls['lastTpl'], array(
				'offset'=> $offset, 
	...
	private function _parse_pagination_links() {
		...
		for ( $page = $this->lowest_visible_page; $page <= $this->highest_visible_page; $page++ ) {
			...
			if ( $page == $this->current_page ) {
				$output .= $this->_parse( static::$tpls['currentPageTpl'], array('offset'=> $offset, 'page_number'=> $page));
			} else {
				$output .= $this->_parse(static::$tpls['pageTpl'], array('offset'=> $offset, 'page_number'=> $page));
			}
		}
		return $output;
	}
	...
	private function _parse_nextTpl() {
		...
		$offset = $this->page_to_offset( $page_number, $this->results_per_page );
		if ( $this->current_page < $this->page_count ) {
			return $this->_parse(static::$tpls['nextTpl'], array('offset'=> $offset, 'page_number'=> $page_number));
		} else {
			return '';
		}
	}
	...
	private function _parse_prevTpl() {
		...
		$offset = $this->page_to_offset( $page_number, $this->results_per_page );
		if ($this->offset > 0) {
			return $this->_parse( static::$tpls['prevTpl'], array('offset'=> $offset, 'page_number'=> $page_number) );
		}
	}
	 ...
	 */
	public static function links($record_count, $base_url='?') {
		...
		$first_pass = $this->_parse(static::$tpls['outerTpl'], $this->properties);
	 ...
	 */
	public function setTpl($tpl, $content) {
